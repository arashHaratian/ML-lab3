...
max_distance <- max(distHaversine(target_loc, data_full[,  c("longitude", "latitude")]))/1000 

#gives the maximum date difference to "previous date"
max_date_diff <- max(as.vector(difftime(target_date,data_full$date, units = "day")))
```
...
1. Kernel width of physical distance(h_distance=50000), which means 50 km. This is to gives larger kernel value to near by stations. As Figure 1 shown below, if the data is from any station located more than 100km away from the target location, the data will not affect the prediction much. Note: In Figure 1, only kernel values within 200km are presented. As the true maximum distance to the target station ( `r max_distance`km) is way to far and the kernel value is already small after 100km.
  
2. Kernel width of date difference(h_date <- 132.5), which means half a year. This is to gives larger kernel value to the observation within same year, especially for the month that is within half year difference to the target date. As Figure 2 shown below, as the date difference approach 500 days the kernel value become very small. Note: In Figure 2, only kernel values within 1500 days are presented. As the true maximum date difference to the target date ( `r max_date_diff` days) is way to high and the kernel value is already small after 500 days.

3. Kernel width of hour difference(h_time <- 6), which means 6 hours. This will gives larger kernel value to the observation within 6 hours, and lower kernel value for more than 12 hours difference. This is to account for the difference of day time and night time.

```{r , echo=F}
# Check if the weight is suitable for our application
h_distance <- 50000 ; h_date <- 132.5; h_time <- 6
distance <- seq(0,max_distance,1)/h_distance
k_dist <- exp(-(distance)^2)

plot(k_dist, type = "l", xlab = "Phycical Distance", ylab = "kernel",xlim=c(0,200000),main="Figure 1: Kernal value with respect to physical distance")
# Since some of the other stations are too far away, limit the ploting x-value to make observation easier
...
k_date <- exp(-(date)^2)
plot(k_date, type= "l", xlab= "Date", ylab = "kernel",xlim=c(0,1500),main="Kernal value with respect to date distance") # we only present t closest 2000 day, since the day that is before that has too little affect to the vaule, which is also what we want


# kernel of time
time <- seq(0,24,1)/h_time
k_time <- exp(-(time)^2)
plot(k_time, xlab = "Time", ylab = "kernel",xlim=c(0,24), type = "l",main="Kernal value with respect to hour distance")
```
...
for(i in seq_along(hours)){
  ...
  date_distance <- difftime(target_date, available_data$date, units = "day")
  hour_distance <- difftime(as.POSIXct(hours[i], format = "%H:%M:%S"),
                     as.POSIXct(available_data$time, format = "%H:%M:%S"),
                     units = "hour")
  
  weight <- exp(-(physcial_distance/h_distance)^2) + exp(-(as.numeric(date_distance)/h_date)^2) + exp((-as.numeric(hour_distance)/h_time)^2)
  # weight <- exp(-physcial_distance/h_distance) + exp(-as.numeric(date_distance)/h_date) + exp(-as.numeric(hour_distance)/h_time)
